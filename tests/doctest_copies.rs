...
fn test_encode_lower() {
    assert_eq!(
        base16::encode_lower(b"Hello World"),
...
fn test_encode_upper() {
    assert_eq!(
        base16::encode_upper(b"Hello World"),
        "48656C6C6F20576F726C64"
    );
...
fn test_encode_config() {
    ...
    assert_eq!(
        base16::encode_config(&data, base16::EncodeLower),
        "010203aabbcc"
    );
    assert_eq!(
        base16::encode_config(&data, base16::EncodeUpper),
        "010203AABBCC"
    );
}
...
fn test_encode_config_buf() {
    ...
    for msg in messages {
        let bytes_written =
            base16::encode_config_buf(msg.as_bytes(), base16::EncodeUpper, &mut buffer);
...
fn test_encode_config_slice() {
    ...
    let wrote = base16::encode_config_slice(message, base16::EncodeLower, &mut buffer);

    assert_eq!(message.len() * 2, wrote);
    assert_eq!(
        core::str::from_utf8(&buffer[..wrote]).unwrap(),
        "57752d54616e67204b696c6c612042656573"
    );

    // Appending to an existing buffer is possible too.
    let wrote2 =
        base16::encode_config_slice(b": The Swarm", base16::EncodeLower, &mut buffer[wrote..]);
    let write_end = wrote + wrote2;
    assert_eq!(
        core::str::from_utf8(&buffer[..write_end]).unwrap(),
...
fn test_decode() {
    assert_eq!(
        &base16::decode("48656c6c6f20576f726c64".as_bytes()).unwrap(),
        b"Hello World"
    );
    assert_eq!(
        &base16::decode(b"deadBEEF").unwrap(),
        &[0xde, 0xad, 0xbe, 0xef]
    );
    // Error cases:
    assert_eq!(
        base16::decode(b"Not Hexadecimal!"),
...
fn test_decode_slice() {
    ...
    assert_eq!(
        &buf[..38],
        b"Good rust libraries use silly examples".as_ref()
    );
